...
exports.createPayment = function(req, res) {
    paymentDetail.find({}).sort({duration: 1}).exec(function(err, data) {
        if(err) {
...
exports.getAllPackage = function(req, res) {
    paymentDetail.find({}).sort({grade: 1}).exec(function(err, data) {
        if(err) {
            res.status(500).json(err);
        } else {
            res.status(200).json(data);
...
exports.updatePackage = function(req, res) {
    paymentDetail.findOne({'_id': req.params.id}).select().exec(function(err, data) {
        if(err) {
            res.status(500).json(err);
        } else {
            if(data.length !== 0) {
...
exports.deletePackage = function(req, res) {
    paymentDetail.findByIdAndRemove({'_id': req.params.id}, function(err, data) {
        if(err) {
            res.status(500).json(err);
        } else {
            paymentDetail.find({}).sort({grade: 1}).exec(function(err, data) {
